    ...
    while (1) {
        ...
        switch (choice) {
            case 0:
                ...
                FreeList(Head);   // FIX: Free all memory before exit instead of just calling exit()
                return 0;
            ...
            case 4:
                ...
                {
                    char* Initial = (char*)malloc(sizeof(char));
                    if (!Initial) { printf("Memory allocation failed.\n"); break; }
                    ...
                    if (found != NULL) {
                        ...
                        printf("-------------------------------------------------------------------------------------------------------------------\n");
                        printf("%-45s %-25s %-25s %-25s %-30s %-30s\n",
                            found->name, found->initial, found->address, found->phone_number,
                            GetDesignationString(found->designation),
                            GetDepartmentString(found->department));
                    } else {
                        printf("\nFaculty not found.\n");
                    }
                    free(Initial);
